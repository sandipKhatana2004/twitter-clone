...
function TweetCard({id, user, content, createdAt, likeCount, likedByMe}: Tweet) {
    return( 
    <li className="flex gap-4 border-b py-4 px-4">
...
function HeartButton({likedByMe, likeCount} : HeartButtonProps) {
    ...
    return (
        <button className={`group flex items-center gap-1 self-start  transition-colors duration-200 -ml-2 ${likedByMe ? "text-red-500" : "text-gray-500 hover:text-red-500 focus-visible:text-red-500"}`}>
            <IconHoverEffect>
